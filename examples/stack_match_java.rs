...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let base_data_dir = std::env::current_dir()?.join(file!()).parent().ok_or("Get parent failed")?.join("data/stack_match_java");
    let synthesis_template = base_data_dir.join("Synthesis.java");
    let lib_src_file = base_data_dir.join("Stack.java");
    ...
    let mut config_builder = CEGISConfigParamsBuilder::default()
        ...
        .lib_func("Stack()".into())
        .lib_func("Stack.push(int)".into())
        .lib_func("Stack.pop()".into())
    ...
    let (mut record_file, record_file_path) = TempFileBuilder::new().prefix("stack_match_java.").suffix(".record.json").tempfile_in(".")?.keep()?;
