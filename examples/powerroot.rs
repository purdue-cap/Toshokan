...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let config = CEGISConfig::new(
        ...
        10,
        vec![ExcludedHole::Position(7, -1), ExcludedHole::Position(8, -1)].into_iter(),
        true,
        true,
        log_level == LevelFilter::Trace,
        synthesis.as_path(),
    ...
    let (mut record_file, record_file_path) = Builder::new().prefix("powerroot.").suffix(".record.json").tempfile_in(".")?.keep()?;
