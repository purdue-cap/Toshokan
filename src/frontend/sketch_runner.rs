...
impl SketchRunner{
    ...
    pub fn output<P: AsRef<Path>>(&mut self, input_file:P) -> io::Result<Output> {
        ...
        debug!(target: "SketchRunner", "Sketch command: {:?}", self.sketch);
        let result = self.sketch.output();
        trace!(target: "SketchRunner", "Sketch result.status: {:?}", result.as_ref().ok().map(|r| r.status));
        trace!(target: "SketchRunner", "Sketch result.stdout: {}",
            result.as_ref().ok().map(|r| &r.stdout)
            .and_then(|b| String::from_utf8(b.clone()).ok())
            .unwrap_or("<Failure>".to_string())
        );
        trace!(target: "SketchRunner", "Sketch result.stderr: {}",
