...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let base_data_dir = std::env::current_dir()?.join(file!()).parent().ok_or("Get parent failed")?.join("data/evalpoly_combined_java");
    ...
    let (mut record_file, record_file_path) = TempFileBuilder::new().prefix("evalpoly_combined_java.").suffix(".record.json").tempfile_in(".")?.keep()?;
