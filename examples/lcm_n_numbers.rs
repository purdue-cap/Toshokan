...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let base_data_dir = PathBuf::from(file!()).parent().ok_or("Get parent failed")?.join("data/lcm_n_numbers");
    ...
    let config = CEGISConfig::new(
        ...
        &["nums_0_5_5_0","nums_1_6_6_0","nums_2_7_7_0","nums_3_8_8_0","nums_4_9_9_0"], Some(5.0));
    let mut main_loop = CEGISLoop::new(config);

    println!("{}", main_loop.run_loop()?.or(Some("Unsolvable benchmark".to_string())).unwrap());
    let (mut record_file, record_file_path) = Builder::new().prefix("lcm_n_numbers.").suffix(".record.json").tempfile_in(".")?.keep()?;
