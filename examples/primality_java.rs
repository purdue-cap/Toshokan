...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let base_data_dir = std::env::current_dir()?.join(file!()).parent().ok_or("Get parent failed")?.join("data/primality_java");
    ...
    let api_src_file = base_data_dir.join("Primality.java");
    ...
    let (mut record_file, record_file_path) = TempFileBuilder::new().prefix("primality_java.").suffix(".record.json").tempfile_in(".")?.keep()?;
