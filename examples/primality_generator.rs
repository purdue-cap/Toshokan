...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let base_data_dir = PathBuf::from(file!()).parent().ok_or("Get parent failed")?.join("data/primality_generator");
    ...
    let config = CEGISConfig::new(
        ...
        10,
        vec![ExcludedHole::Position(6, -1), ExcludedHole::Position(7, -1)].into_iter(),
        true,
        true,
        log_level == LevelFilter::Trace,
        synthesis.as_path(),
    ...
    let (mut record_file, record_file_path) = Builder::new().prefix("primality_generator.").suffix(".record.json").tempfile_in(".")?.keep()?;
