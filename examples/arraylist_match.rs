...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let base_data_dir = PathBuf::from(file!()).parent().ok_or("Get parent failed")?.join("data/arraylist_match");
    ...
    let config = CEGISConfig::new_full_config(
        ...
        impl_file.as_path(),
        &[("ANONYMOUS::arraylist_push_back_real", FuncConfig::NonPure{args:2, state_arg_idx: 0}),
        ("ANONYMOUS::arraylist_get_real", FuncConfig::StateQuery{args:2, state_arg_idx: 0}),
        ("ANONYMOUS::arraylist_new_real", FuncConfig::Init{args: 0})],
        &[],
        "main",
        3,
    ...
    let (mut record_file, record_file_path) = Builder::new().prefix("arraylist_match.").suffix(".record.json").tempfile_in(".")?.keep()?;
