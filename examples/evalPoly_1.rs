...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let base_data_dir = PathBuf::from(file!()).parent().ok_or("Get parent failed")?.join("data/evalPoly_1");
    ...
    let (mut record_file, record_file_path) = Builder::new().prefix("evalPoly_1.").suffix(".record.json").tempfile_in(".")?.keep()?;
