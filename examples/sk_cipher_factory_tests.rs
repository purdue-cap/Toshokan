...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let base_data_dir = PathBuf::from(file!()).parent().ok_or("Get parent failed")?.join("data/sk_CipherFactoryTests");
    ...
    let config = CEGISConfig::new(
        ...
        impl_file.as_path(),
        &[("Cipher::doFinal_byte", 2)],
        "CipherFactoryTests::main",
        1,
    ...
    let (mut record_file, record_file_path) = Builder::new().prefix("sk_CipherFactoryTests.").suffix(".record.json").tempfile_in(".")?.keep()?;
