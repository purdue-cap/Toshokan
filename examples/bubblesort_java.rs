...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let base_data_dir = std::env::current_dir()?.join(file!()).parent().ok_or("Get parent failed")?.join("data/bubblesort_java");
    ...
    let api_src_file = base_data_dir.join("BubbleSort.java");
    ...
    let mut config_builder = CEGISConfigParamsBuilder::default()
        ...
        .lib_func("ArrayListP.add(int)".into())
        .lib_func("ArrayListP.get(int)".into())
        .lib_func("ArrayListP.set(int, int)".into())
        // .lib_func("ArrayListP.size()".into())
        ...
        .output_class("BubbleSort".into())
        .n_inputs(3 as usize)
        .hist_cap_padding(64 as usize)
        .n_unknowns(16 as usize)
        .output_dir("results/")
        .keep_tmp(true)
        .enable_record(true);
    ...
    let (mut record_file, record_file_path) = TempFileBuilder::new().prefix("bubblesort_java.").suffix(".record.json").tempfile_in(".")?.keep()?;
