...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let base_data_dir = PathBuf::from(file!()).parent().ok_or("Get parent failed")?.join("data/stack_match");
    ...
    let (mut record_file, record_file_path) = Builder::new().prefix("stack_match.").suffix(".record.json").tempfile_in(".")?.keep()?;
