...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let base_data_dir = PathBuf::from(file!()).parent().ok_or("Get parent failed")?.join("data/heap_test_param");
    ...
    let (mut record_file, record_file_path) = Builder::new().prefix("heap_test_param.").suffix(".record.json").tempfile_in(".")?.keep()?;
