...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let base_data_dir = std::env::current_dir()?.join(file!()).parent().ok_or("Get parent failed")?.join("data/set_match_java");
    let synthesis_template = base_data_dir.join("Synthesis.java");
    let lib_src_file = base_data_dir.join("Set.java");
    ...
    let mut config_builder = CEGISConfigParamsBuilder::default()
        ...
        .lib_func("Set()".into())
        .lib_func("Set.size()".into())
        .lib_func("Set.contains(int)".into())
        .lib_func("Set.add(int)".into())
    ...
    let (mut record_file, record_file_path) = TempFileBuilder::new().prefix("set_match_java.").suffix(".record.json").tempfile_in(".")?.keep()?;
